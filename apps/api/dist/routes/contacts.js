import { z } from 'zod';
import { NotFoundError, ForbiddenError, ValidationError, ConflictError, ContactRequestRequiredError, ContactRequestPendingError, } from '../lib/errors.js';
...
export default async function contactRoutes(fastify) {
    ...
    fastify.post('/contacts/requests', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        // Check permission
        const permResult = await permissionService.canSendMessage({ userId: user.userId, tenantId: user.tenantId }, { userId: toUserId });
        // If already allowed, no request needed
        if (permResult.allowed) {
            throw new ConflictError('Kontaktanfrage nicht erforderlich - Sie können diesen Benutzer bereits kontaktieren');
        }
    ...
    fastify.post('/contacts/requests/:id/respond', { preHandler: [authenticate] }, async (request, reply) => {
        const user = request.user;
    ...
    // Unblock a user
    fastify.delete('/contacts/block/:userId', { preHandler: [authenticate] }, async (request, reply) => {
        const user = request.user;
    ...
    fastify.get('/contacts/can-message/:userId', { preHandler: [authenticate] }, async (request, reply) => {
        const user = request.user;
        const { userId: targetUserId } = request.params;
        const result = await permissionService.canSendMessage({ userId: user.userId, tenantId: user.tenantId }, { userId: targetUserId });
