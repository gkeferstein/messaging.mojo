...
export default async function conversationRoutes(fastify) {
    ...
    fastify.get('/conversations', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        const result = await messagingService.getConversations(user.userId, query.limit, query.cursor);
    ...
    fastify.post('/conversations', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        try {
            const conversation = await messagingService.createConversation({ userId: user.userId, tenantId: user.tenantId }, {
                type: input.type,
                name: input.name,
    ...
    fastify.get('/conversations/:id/participants', { preHandler: [authenticate] }, async (request, reply) => {
        const user = request.user;
        const { id } = request.params;
    ...
    fastify.post('/conversations/:id/read', { preHandler: [authenticate] }, async (request, reply) => {
        const user = request.user;
        const { id } = request.params;
