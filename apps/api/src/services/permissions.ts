import { MessagingRule, ContactRequest, BlockedUser } from '@prisma/client';
...
export class PermissionService {
  ...
  async canSendMessage(
    ...
    const hasApprovedContact = await this.hasApprovedContact(
      sender.userId,
      recipient.userId
    );
    ...
    for (const rule of rules) {
      ...
      if (matches) {
        if (rule.requireApproval) {
          ...
          const pendingRequest = await this.getPendingRequest(
            sender.userId,
            recipient.userId
          );
        ...
        if (rule.maxMessagesPerDay) {
          const messageCount = await this.getMessageCountToday(
            sender.userId,
            recipient.userId
          );
   ...
   */
  async isConversationAdmin(userId: string, conversationId: string): Promise<boolean> {
    const participant = await prisma.participant.findUnique({
      where: {
        conversationId_userId: { conversationId, userId },
      },
  ...
  private async hasApprovedContact(userId1: string, userId2: string): Promise<boolean> {
  ...
  private async getActiveRules(): Promise<MessagingRule[]> {
    ...
    if (dbRules.length === 0) {
      return DEFAULT_RULES as unknown as MessagingRule[];
    }
  ...
  private matchesRule(
    ...
    const sourceRoles = rule.sourceRoles as string[];
    const targetRoles = rule.targetRoles as string[];

    // Prüfe Source-Scope
    if (rule.sourceScope === 'tenant') {
      // Sender muss Tenant haben und passende Rolle
      if (!sender.tenantId || !sender.tenantRole) {return false;}
      if (!sourceRoles.includes(sender.tenantRole)) {return false;}
    } else if (rule.sourceScope === 'platform') {
      // Sender braucht entweder Tenant-Rolle oder Platform-Rolle
      const hasRole = 
        (sender.tenantRole && sourceRoles.includes(sender.tenantRole)) ||
    ...
    if (rule.targetScope === 'tenant') {
      ...
      if (sender.tenantId !== recipient.tenantId) {return false;}
      if (!recipient.tenantRole) {return false;}
      if (!targetRoles.includes(recipient.tenantRole)) {return false;}
    } else if (rule.targetScope === 'platform') {
      // Recipient braucht passende Rolle
      const hasRole = 
        (recipient.tenantRole && targetRoles.includes(recipient.tenantRole)) ||
  ...
  private async getMessageCountToday(
    senderId: string,
    recipientId: string
  ): Promise<number> {
